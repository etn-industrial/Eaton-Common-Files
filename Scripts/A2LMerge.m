...
MergedAxisPointName = {};          % Cell array of strings of the names of the Axis Points' Name in the A2L files
MergedAxisPoint = {};              % Cell array of strings of the names of the Axis Points in the A2L files
MergedAxisPointRecordLayout = {};  % Cell array of strings of the names of the Axis Points' Record Layouts in the A2L files
...
MergedAxisPointRecordLayoutInfo = {};  % Container for Record Layout objects.

rootFolder = fileparts(which('M3Inverter_startup.m'));
ASWFolder = fullfile(rootFolder,'Release','ASW_Modules','Eaton');
A2LFiles = FindA2LFiles(A2LFolder);

for fl=1:length(A2LFiles)
    a2lfile = Eaton_xcpA2L(string(A2LFiles(fl)));
    ...
    for ii=1:length(a2lfile.Characteristics)
        ...
        if ~strcmp(MergedCharacteristics, str)
            ...
            %Merge Axis Points' Record Layout
            if ~isempty(CharacteristicsInfo.AxisDescr)
                AxisDescObj = a2lfile.AxisInfo(CharacteristicsInfo.AxisDescr.AXIS_PTS_REF.AxisPoints);
                AxisDescLayout = AxisDescObj.Deposit;
                if ~ismember(AxisDescLayout.Name,MergedAxisPointRecordLayout)
                    MergedAxisPointRecordLayout{end + 1} = AxisDescLayout.Name;
                    MergedAxisPointRecordLayoutInfo{end + 1} = AxisDescLayout;
                end
                AxisPointName = CharacteristicsInfo.AxisDescr.AXIS_PTS_REF.AxisPoints;
                if ~ismember(AxisPointName,MergedAxisPointName)
                    MergedAxisPointName{end + 1}= AxisPointName;
                    MergedAxisPoint{end + 1} = a2lfile.AxisInfo(AxisPointName);
                end
            end
            %Merge Compu Method
...
fid = fopen(fullfile(ASWFolder,ASAP2File),'w+');
fprintf(fid, '/******************************************************************************\n');
fprintf(fid, '*\n');
...
for ii=1:length(MergedAxisPointRecordLayoutInfo)
    fprintf(fid, '\n\t\t/begin RECORD_LAYOUT %s', MergedAxisPointRecordLayoutInfo{1, ii}.Name);
    fprintf(fid, '\n\t\t\t%s %d %s %s %s %s', MergedAxisPointRecordLayoutInfo{1, ii}.Records{1,1}.Name,MergedAxisPointRecordLayoutInfo{1, ii}.Records{1,1}.Position,MergedAxisPointRecordLayoutInfo{1, ii}.Records{1,1}.DataType,MergedAxisPointRecordLayoutInfo{1, ii}.Records{1,1}.IndexMode,MergedAxisPointRecordLayoutInfo{1, ii}.Records{1,1}.IndexOrder,MergedAxisPointRecordLayoutInfo{1, ii}.Records{1,1}.AddressType);   
    fprintf(fid, '\n\t\t/end   RECORD_LAYOUT');
    fprintf(fid, '\n');
end
...
for ii=1:length(MergedCharacteristicsInfo)
    ...
    fprintf(fid, '\n\t\t\t/* Array Size             */ \n\t\t\t NUMBER                       %d',MergedCharacteristicsInfo{1, ii}.Dimension(1));    
    end
    
    if ~isempty(MergedCharacteristicsInfo{1, ii}.AxisDescr)
        fprintf(fid, '\n\t\t/begin AXIS_DESCR ');
        fprintf(fid, '\n\t\t/* Description of X-Axis Points */ ');
        fprintf(fid, '\n\t\t\t/* Axis Type            */      %s',MergedCharacteristicsInfo{1, ii}.AxisDescr.Attribute); 
        fprintf(fid, '\n\t\t\t/* Reference to Input   */      %s',MergedCharacteristicsInfo{1, ii}.AxisDescr.InputQuantity); 
        fprintf(fid, '\n\t\t\t/* Conversion Method    */      %s',MergedCharacteristicsInfo{1, ii}.AxisDescr.Conversion); 
        fprintf(fid, '\n\t\t\t/* Number of Axis Pts   */      %s',strjoin(string(num2str(MergedCharacteristicsInfo{1, ii}.AxisDescr.MaxAxisPoints)))); 
        fprintf(fid, '\n\t\t\t/* Lower Limit          */      %s',strjoin(string(num2str(MergedCharacteristicsInfo{1, ii}.AxisDescr.LowerLimit)))); 
        fprintf(fid, '\n\t\t\t/* Upper Limit          */      %s',strjoin(string(num2str(MergedCharacteristicsInfo{1, ii}.AxisDescr.UpperLimit))));
        fprintf(fid, '\n\t\t\tAXIS_PTS_REF      %s',MergedCharacteristicsInfo{1, ii}.AxisDescr.AXIS_PTS_REF.AxisPoints);
        fprintf(fid, '\n\t\t/end AXIS_DESCR ');
    end
    fprintf(fid, '\n\t\t/end   CHARACTERISTIC');
    fprintf(fid, '\n');
...
for ii=1:length(MergedAxisPoint)
    fprintf(fid, '\n\t\t/begin AXIS_PTS');
    fprintf(fid, '\n\t\t\t/* Name                   */      %s',MergedAxisPoint{1, ii}.Name);
    fprintf(fid, '\n\t\t\t/* Long Identifier        */      %s',['"',MergedAxisPoint{1, ii}.LongIdentifier,'"']);
    fprintf(fid, '\n\t\t\t/* ECU Address            */      0x%04x /* @ECU_Address@%s@ */',MergedAxisPoint{1, ii}.ECUAddress,MergedAxisPoint{1, ii}.Name);
    fprintf(fid, '\n\t\t\t/* Input Quantity         */      %s',MergedAxisPoint{1, ii}.InputQuantity);
    fprintf(fid, '\n\t\t\t/* Record Layout          */      %s',MergedAxisPoint{1, ii}.Deposit.Name);
    fprintf(fid, '\n\t\t\t/* Maximum Difference     */      0');
    fprintf(fid, '\n\t\t\t/* Conversion Method      */      %s',MergedAxisPoint{1, ii}.Conversion.Name);
    fprintf(fid, '\n\t\t\t/* Number of Axis Pts     */      %s',strjoin(string(num2str(MergedAxisPoint{1, ii}.Dimension))));
    fprintf(fid, '\n\t\t\t/* Lower Limit            */      %s',strjoin(string(num2str(MergedAxisPoint{1, ii}.LowerLimit))));
    fprintf(fid, '\n\t\t\t/* Upper Limit            */      %s',strjoin(string(num2str(MergedAxisPoint{1, ii}.UpperLimit))));
    fprintf(fid, '\n\t\t/end AXIS_PTS');
    fprintf(fid, '\n');
end
