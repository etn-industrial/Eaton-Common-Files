    ...
    methods (Access = {?UnitTestFactory})
        function InitProperties(obj,SimResults,Data)
            if ~obj.testReportError
                ...
                obj.coverageTable = SimResults.coverageTable;
                obj.mergedCoverageTable = SimResults.mergedCoverageTable;
            end
        end
        ...
        function createTestReportContent(obj)
            ...
            if ~obj.testReportError && ~obj.testHarnessError
                try
                    ...
                    addChapterSection(obj,obj.overallResultsChapter,[obj.testHarnessName,' - Test Coverage'],obj.coverageTable);
            ...
            if ~obj.testReportError && ~obj.testHarnessError
                try
                    ...
                    for rowCnt = 1:numberOfRows
                        if strcmp(string(obj.tableTestCase.Result(rowCnt)),"Pass")
                            ResultIcon =  getPassIcon(obj);
                        elseif strcmp(string(obj.tableTestCase.Result(rowCnt)),"Fail")
                            ResultIcon =  getFailIcon(obj);
                            failedResults{length(failedResults)+1,1} = string(obj.tableTestCase.CurrentValue(rowCnt));
                        elseif strcmp(string(obj.tableTestCase.Result(rowCnt)),"Untested")
                            ResultIcon =  getUntestedIcon(obj);
                        end
                        if ~ismissing(obj.tableTestCase.TestCase(rowCnt))&&~ismissing(obj.tableTestCase.TestStep(rowCnt)) %New Test Case
                            headerRowTitle = {char(["Test Step: ",string(obj.tableTestCase.TestStep(rowCnt))]),'',''};
        ...
        function addTestCaseInputs(obj,tcStartRow,tcEndRow,rowIdx,resultIcon,testStepTable,testStepCounter)
            if ~obj.testReportError && ~obj.testHarnessError
                try
                      ...
                      tableEntries = {'Expected Values:','','Results'};
                      appendNewRowToTable (obj,testStepTable,tableEntries,rowIdx,true)
            ...
            if ~obj.testReportError && ~obj.testHarnessError
                try
                    ...
                    if mod(rowIdx,2)
                       ...
                       newRow.Style = {BackgroundColor('#d8e6ff')};
                    end
                    if isHeaderRow
                        newRow.Style= {BackgroundColor('steelblue'),Bold,FontFamily('Times New Roman'),...
        ...
        function newTable = createTestCaseSummaryTable(obj)
            ...
            if ~obj.testReportError && ~obj.testHarnessError
                try
                    ...
                    columnWidth = {'3in','2.9in','0.15in','0.45in'};
                    newTable = createHeaderForTable(obj,headerRowTitle,columnWidth,false);  
        ...
        function newRow = createTestCaseSummaryRow(obj,rowEntries,rowCounter,hiperlinkText,HlinkEnable)
            ...
            if ~obj.testReportError && ~obj.testHarnessError
                try
                    ...
                    TestCaseNameEntry.Style ={InnerMargin('0.1in', '0.1in')};
                    append(newRow,TestCaseNameEntry);
                    descriptionEntry = TableEntry(char(rowEntries{2}));
                    descriptionEntry.Style ={InnerMargin('0in', '0.1in')};
                    append(newRow,descriptionEntry);
                    if strcmp(string(rowEntries{3}),'Pass')
                       iconEntry = TableEntry(getPassIcon(obj));
                    elseif strcmp(string(rowEntries{3}),'Fail')
                       iconEntry = TableEntry(getFailIcon(obj));
                    elseif strcmp(string(rowEntries{3}),'Untested')
                       iconEntry = TableEntry(getUntestedIcon(obj));
                    else
                        %nothing to do
                    end
        ...
        function newTable = createHeaderForTable(obj,headerNames,columnWidth,alignTextToCenter)
            ...
            if ~obj.testReportError && ~obj.testHarnessError
                try
                    ...
                    newTable.TableEntriesStyle={Border('none','white','1'),FontFamily('Times New Roman'),...
                                                FontSize('9pt'),ResizeToFitContents(true),Color('black'),VAlign('middle')};
                    if alignTextToCenter
                        newTable.Header.Style = {BackgroundColor('steelblue'),Bold,Color('white'),HAlign('center')};
                    else
                        newTable.Header.Style = {BackgroundColor('steelblue'),Bold,Color('white')};
                    end
        ...
        function  createMainPage(obj,Report)
            ...
            if ~obj.testReportError && ~obj.testHarnessError
                try
                    ...
                    title.Style = {FontFamily('Times New Roman'),HAlign('center'),FontSize('24pt')};
        ...
        function failIcon = getUntestedIcon(obj)
             import mlreportgen.dom.*
             try
                 %create an image object for the icon
                 failIcon =Image(fullfile([obj.Error.scriptFolder,'\Icons\ResultsStatusIconUntested.png']));
             catch ME
                 obj.setErrorMessage(ME);
             end  
        end
               ...
               try
                    if ~isempty(obj.signalBuilderBlock)
                        ...
                        for ii = 1:length(signames)
                            ...
                            print(snapshotFig, '-dpng', [obj.Path,'\Snapshots\',obj.testHarnessName,'Signal_',erase(signames{ii},'.')]);
                            Snapshot = Image(fullfile([obj.Path,'\Snapshots\',obj.testHarnessName,'Signal_',erase(signames{ii},'.'),'.png']));
