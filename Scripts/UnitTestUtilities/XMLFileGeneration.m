classdef XMLFileGeneration < handle
    ...
    methods
        ...
        function Convert_xlsx_to_xml(obj,fileName)
            try
                ...
                obj.totalTestCases = 0;
                obj.linkedTestCasesCount = 0;
    ...
    methods (Access = {?UnitTestFactory}) 
        ...
        function createTestNodes(obj,Root,docNode,data,TopNode)
            ...
            [resultSN rowSN columnLink] = obj.findString(data,'URL');
            %Find the rows and columns where each test case starts
            [TCRow TCColumn] = obj.findTestCases(data);
            obj.totalTestCases = length(TCRow)-1;
            for i= 1:length(TCRow)-1
                if resultSN && contains(data(TCRow(i),columnLink),'https')
                    obj.linkedTestCasesCount = obj.linkedTestCasesCount + 1;
                end
                %Create test node
    ...
    methods (Static,Access = {?UnitTestFactory}) 
